...
public class IMDecoder extends ByteToMessageDecoder {
    //解析IM写一下请求内容的正则  [LOGIN][1570712034096][蓝星花]
    ...
    public IMMessage decode(String msg) {
        ...
        try {
            ...
            long time = 0;
            try {
                time = Long.parseLong(h[1]);
            } catch (Exception e) {
            }
            String nickName = h[2];
            //昵称最多十个字
            nickName = nickName.length() < 10 ? nickName : nickName.substring(0, 9);
            IMP cmd = IMP.getInstance(h[0]);
            if (msg.startsWith("[" + IMP.LOGIN.getName() + "]")) {
                return new IMMessage(cmd.getName(), time, nickName);
            } else if (msg.startsWith("[" + IMP.CHAT.getName() + "]")) {
                return new IMMessage(cmd.getName(), time, nickName, content);
